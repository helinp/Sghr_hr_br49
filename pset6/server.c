...
// defines macros for HTTP request line
#define HTTP_METHOD 0
#define HTTP_REQUEST_TARGET 1
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // extract request's request_line
            ...
            // TODO: validate request_line
            
            // Declaring variables
            const char s[2] = " ";
            char* request_line[3] = {0};
            char *quote, *dot, *question;
            
            // Parsing
            request_line[HTTP_METHOD] = strtok(line, s);
            request_line[HTTP_REQUEST_TARGET] = strtok(NULL, s);
            request_line[HTTP_VERSION] = strtok(NULL, s);
            
            // Searches for " and . characters
            quote = strchr (request_line[HTTP_REQUEST_TARGET], '"');
            dot = strchr (request_line[HTTP_REQUEST_TARGET], '.');
            question = strchr (request_line[HTTP_REQUEST_TARGET], '?');
            
           
            // printf("%s | %s | %s \n", request_line[HTTP_VERSION], request_line[HTTP_REQUEST_TARGET], request_line[HTTP_VERSION]);
            
            // Request line tests 
            if (strcmp(request_line[HTTP_METHOD], "GET") != 0) 
            {
                error(405);
                continue;
            }
            else if (request_line[HTTP_REQUEST_TARGET][0] != '/' || quote != NULL || dot == NULL) 
            {
                error(501);
                continue;
            }
            ...
            else if (strcmp(request_line[HTTP_VERSION], "HTTP/1.1\r\n") != 0) 
            {
                error(505);  
                continue;
            }
            ...
            if (question != NULL)
            {
                ...
                // Gets lenghts
                int request_line_len = strlen(request_line[HTTP_REQUEST_TARGET]);
                int query_len = strlen(query) + 1;
                
                // Removes query from path
                request_line[HTTP_REQUEST_TARGET][request_line_len - query_len] = '\0';
            }

            // TODO: concatenate root and absolute-path
            char path[LimitRequestFieldSize] = "";
            char buffer[LimitRequestFieldSize] = "";
            
            strcpy(path, root);
            strcpy(buffer, request_line[HTTP_REQUEST_TARGET]);
            strcat(path, buffer);
...
const char* lookup(const char* extension)
{
    ...
    else if (strcasecmp(extension, "gif") == 0) return "image/gif";
...
ssize_t parse(void)
{
    ...
    // ensure request isn't already request_line
    if (request != NULL)
    {
        return -1;
    }
